...
import classes from './EmployeeQualifications.module.scss';

class EmployeeQualifications extends Form {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    ...
    const res = await httpService.get(`/employees/${this.props.ippisNo}/qualifications`);
  ...
  mapToViewModel(row) {
    return {
      ...
      qualification: row.qualifications.qualification,
   ...
   */
  getOptionValues() {
    const { qualificationId } = this.state.formData;
    return {
      qualifications: this.state.qualificationOptions.filter(
        option => option.id === qualificationId * 1
      )[0]
    };
  }
  ...
  async updateDataObject(stopProcessing) {
    const res = await httpService.patch(
      `/employees/${this.props.ippisNo}/qualifications/${this.state.rowToPreview.id}`,
      this.state.formData
    );
    ...
    if (res) {
      toast.success('Qualification successfully updated!');
      this.updateTableRow();
      this.closeSideDraw();
      this.resetFormData();
    }
  ...
  async handleDelete(event) {
    if (!this.state.isDeleteting) {
      ...
      if (res) {
        toast.success('Qualification successfully deleted!');
        this.removeTableRow();
  ...
  async addDataObject(stopProcessing) {
    const res = await httpService.post(
      `/employees/${this.props.ippisNo}/qualifications`,
      this.state.formData
    );
      ...
      <div className={classes.Preview}>
        ...
        >
          {this.renderSelect(
            'qualification',
            'qualificationId',
            nameMapper(this.state.qualificationOptions, 'qualification'),
            null,
            null,
            this.state.formData.qualificationId
          )}
  ...
  renderCreateForm() {
    return (
      <form ref={form => (this.Form = form)} onSubmit={this.handleSubmit}>
        <p>Add a new employee qualification</p>
...
export default withRouter(EmployeeQualifications);
