...
export default class EmployeeVerifier extends Component {
    ...
    updateInputValue(value) {
        ...
        if (employee) {
            this.setState({ employee: this.input.value === this.state.employee.ippisNo })
        }
    }

    componentDidMount() {
        this.input = this.verifier.querySelector('input') || this.verifier.querySelector('select') || this.verifier.querySelector('textarea');
    ...
    handleEmployeeSelection({ currentTarget }) {
        ...
        this.setState({ isEmployeeSelected: true, employee: selectedEmployee, employees: [] });
    ...
    async verifyEmployee() {
        ...
        if (this.inputValue.length > 1) {
            ...
            const results = await httpService.get(`/employee?ippisNo=${this.inputValue}`)
    ...
    profileLayout(employee) {
        return (
            <React.Fragment>
                ...
                <div>
                    <p className={classes.Name}>{truncate(`${employee.firstName} ${employee.lastName} ${employee.middleName}`, 30)}</p>
        <p className={classes.Department}>{truncate(`${employee.employeeJob.department.description}`, 30)} (<span>{truncate(employee.employeeAppointment.presentJobTitle.description)})</span></p>
                </div>
            </React.Fragment>
        )
    }
        ...
        return !this.props.hideList ? (
            <ul className={classes.VerificationResult}>
                {employees.map((employee, i) => { 
                    return (<li key={i} id={i} onClick={this.handleEmployeeSelection} className={classes.Result}>  
    ...
    render() {
        ...
        return (
            <div className={classes.Verifier} ref={verifier => this.verifier = verifier}>
                {this.props.children}
